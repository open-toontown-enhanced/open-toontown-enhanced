...
class DistributedToonHallInteriorAI(DistributedToonInteriorAI):
    ...
    def __init__(self, block: int, air, zoneId: int, building):
    ...
    def logToonEntered(self, avId: int, zoneId: int):
    ...
    def logToonLeft(self, avId: int, zoneId: int):
        result = self.getCurPhase()
        if result == -1:
            phase = 'not available'
        else:
