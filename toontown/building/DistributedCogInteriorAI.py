...
class DistributedCogInteriorAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def __init__(self, air, elevator):
        ...
        self.savedByMap = {}
        self.fsm = ClassicFSM.ClassicFSM('DistributedCogInteriorAI', [
    ...
    def getCogs(self):
        ...
        return [cogIds, reserveIds, values]
    ...
    def setAvatarJoined(self):
        ...
        if self.toons.count(avId) == 0:
            self.air.writeServerEvent('suspicious', avId, 'DistributedCogInteriorAI.setAvatarJoined from toon not in %s.' % self.toons)
    ...
    def handleAllAboard(self, seats):
        ...
        if __dev__:
            ...
            if not currState == 'Resting':
                avId = self.air.getAvatarIdFromSender()
                self.air.writeServerEvent('suspicious', avId, 'unexpected state for DistributedCogInteriorAI.handleAllAboard(). Current State = %s.' % currState)
                self.notify.warning('unexpected state for DistributedCogInteriorAI.handleAllAboard(). Current State = %s' % currState)
                return
