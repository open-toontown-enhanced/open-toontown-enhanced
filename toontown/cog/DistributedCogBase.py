...
class DistributedCogBase(DistributedAvatar.DistributedAvatar, Suit.Suit, CogBase.CogBase):
    ...
    def __init__(self, cr):
        try:
            self.DistributedCogBase_initialized
            return
        except:
    ...
    def delete(self):
        try:
            self.DistributedCogBase_deleted
        except:
            self.DistributedCogBase_deleted = 1
    ...
    def beginSupaFlyMove(self, pos, moveIn, trackName):
        ...
        timeTillLanding = CogTimings.fromSky - impactLength
    ...
    def setBrushOff(self, index):
        self.setChatAbsolute(CogDialog.getBrushOffText(self.getStyleName(), index), CFSpeech | CFTimeout)
