...
from toontown.cog import DistributedCashbotBossGoonAI
from toontown.coghq import DistributedCashbotBossTreasureAI
from toontown.battle import BattleExperienceAI
from toontown.chat import ResistanceChat
from direct.fsm import FSM
from toontown.cog import DistributedBossCogAI
import random, math
import functools
...
class DistributedCashbotBossAI(DistributedBossCogAI.DistributedBossCogAI, FSM.FSM):
    ...
    def generateCogs(self, battleNumber):
        cogs = self.invokeCogPlanner(11, 0)
        skelecogs = self.invokeCogPlanner(12, 1)
        activeCogs = cogs['activeCogs'] + skelecogs['activeCogs']
        ...
        reserveCogs.sort(key=functools.cmp_to_key(compareJoinChance))
        return {'activeCogs': activeCogs, 'reserveCogs': reserveCogs}
    ...
    def enterVictory(self):
        self.resetBattles()
        self.cogsKilled.append({'type': None, 'level': None, 'track': self.dna.dept, 'isSkelecog': 0, 'isForeman': 0, 'isVP': 0, 'isCFO': 1, 'isSupervisor': 0, 'isVirtual': 0, 'activeToons': self.involvedToons[:]})
