...
class DistributedLawbotBossCog(DistributedCogBase.DistributedCogBase):
    notify = DirectNotifyGlobal.directNotify.newCategory('DistributedLawbotBossCog')
    timeToShow = 1.0
    timeToRelease = 3.15
    ...
    def __init__(self, cr):
        ...
        try:
            ...
            self.boss = None
            self.fsm = ClassicFSM.ClassicFSM('DistributedLawbotBossCog', [
                State.State('Off',
                            self.enterOff,
                            self.exitOff, [
                                'Walk',
    ...
    def enterNeutral(self):
        ...
        self.notify.debug('DistributedLawbotBossCog: Neutral')
        self.loop('neutral', 0)
    ...
    def makeProsecuteThrowingTrack(self, evidence, inFlightDuration, hitPos):
        cogTrack = Sequence()
        cogTrack.append(ActorInterval(self, 'throw-paper'))
        throwPaperDuration = cogTrack.getDuration()
        ...
        throwingTrack = Parallel(cogTrack, propTrack, rotateTrack)
    ...
    def makePreThrowAttackTrack(self, evidence, inFlightDuration, hitPos):
        cogTrack = Sequence()
        cogTrack.append(ActorInterval(self, 'throw-paper', endTime=self.timeToRelease))
        throwPaperDuration = cogTrack.getDuration()
        origHpr = self.getHpr()
        self.headsUp(hitPos)
        newHpr = self.getHpr()
    ...
    def makePostThrowAttackTrack(self, evidence, inFlightDuration, hitPos):
        cogTrack = Sequence()
        cogTrack.append(ActorInterval(self, 'throw-paper', startTime=self.timeToRelease))
