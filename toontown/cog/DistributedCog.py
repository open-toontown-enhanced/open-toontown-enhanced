...
from . import CogBase
from . import DistributedCogPlanner
from direct.directnotify import DirectNotifyGlobal
from . import CogDialog
...
class DistributedCog(DistributedCogBase.DistributedCogBase, DelayDeletable):
    notify = DirectNotifyGlobal.directNotify.newCategory('DistributedCog')
        ...
        self.fsm = ClassicFSM.ClassicFSM('DistributedCog', [
         ...
         State.State('FromSky',
                     ...
                     self.exitFromSky, [
                         ...
                         'ToSky',
                         'WalkFromStreet']),
         State.State('FromCogBuilding',
                     self.enterFromCogBuilding,
                     self.exitFromCogBuilding, [
                         'WalkToStreet',
                         'Walk',
                         'Battle',
                         'neutral',
         ...
         State.State('ToToonBuilding',
                     ...
                     self.exitToToonBuilding, [
                         'neutral',
                         'Battle']),
         State.State('ToCogBuilding',
                     self.enterToCogBuilding,
                     self.exitToCogBuilding, [
                         'neutral',
                         'Battle']),
         State.State('ToCogHQ',
                     self.enterToCogHQ,
    ...
    def denyBattle(self):
        DistributedCogBase.DistributedCogBase.denyBattle(self)
    ...
    def beginBuildingMove(self, moveIn, doneEvent, cog = 0):
        ...
        if moveIn:
            ...
            return self.beginMove(doneEvent, doorPt, buildingPt, time=CogTimings.fromCogBuilding)
        return None
    ...
    def enterFromCogBuilding(self, leg, time):
        ...
        h = self.calculateHeading(a, b)
        self.setHprScale(h, 0.0, 0.0, 1.0, 1.0, 1.0)
        self.mtrack = Sequence(LerpPosInterval(self, leg.getLegTime(), b1, startPos=a1), name=self.taskName('fromCogBuilding'))
        self.mtrack.start(time)

    def exitFromCogBuilding(self):
        self.disableBattleDetect()
        self.mtrack.finish()
        del self.mtrack
    ...
    def enterToCogBuilding(self, leg, time):
        ...
        h = self.calculateHeading(a, b)
        self.setHprScale(h, 0.0, 0.0, 1.0, 1.0, 1.0)
        self.mtrack = Sequence(LerpPosInterval(self, leg.getLegTime(), b1, startPos=a1), name=self.taskName('toCogBuilding'))
        self.mtrack.start(time)
    ...
    def enterBattle(self):
        DistributedCogBase.DistributedCogBase.enterBattle(self)
        self.resumePath(0)

    def enterNeutral(self):
        self.notify.debug('DistributedCog: Neutral (entering a Door)')
        self.resumePath(0)
        self.loop('neutral', 0)
    ...
    def enterWaitForBattle(self):
        DistributedCogBase.DistributedCogBase.enterWaitForBattle(self)
        self.resumePath(0)
