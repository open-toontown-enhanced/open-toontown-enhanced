...
from toontown.cog import DistributedCogAI
...
from toontown.cog import DistributedLawbotBossCogAI
...
class DistributedLawbotBossAI(DistributedBossCogAI.DistributedBossCogAI, FSM.FSM):
    ...
    def generateCogs(self, battleNumber):
        if battleNumber == 1:
            ...
            listVersion = list(CogBuildingGlobals.CogBuildingInfo)
    ...
    def enterVictory(self):
        self.resetBattles()
        self.cogsKilled.append({'type': None, 'level': None, 'track': self.dna.dept, 'isSkelecog': 0, 'isForeman': 0, 'isVP': 1, 'isCFO': 0, 'isSupervisor': 0, 'isVirtual': 0, 'activeToons': self.involvedToons[:]})
    ...
    def giveCogSummonReward(self, toon, prefDeptIndex, prefSummonType):
        cogLevel = int(self.toonLevels / self.maxToonLevels * CogDNA.cogsPerDept)
        cogLevel = min(cogLevel, CogDNA.cogsPerDept - 1)
