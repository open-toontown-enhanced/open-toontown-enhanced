...
class CogPage(ShtikerPage.ShtikerPage):
    ...
    def load(self):
        ...
        del frameModel
        self.title = DirectLabel(parent=self.iconNode, relief=None, text=TTLocalizer.CogPageTitle, text_scale=0.1, text_pos=(0.04, 0), textMayChange=0)
        self.radarButtons = []
    ...
    def addQuotaLabel(self, panel):
        ...
        if base.localAvatar.cogs[index] < COG_COMPLETE1:
            quota = str(COG_QUOTAS[0][index % CogDNA.cogsPerDept])
        else:
            quota = str(COG_QUOTAS[1][index % CogDNA.cogsPerDept])
        quotaLabel = DirectLabel(parent=panel, pos=(0.0, 0.0, -0.215), relief=None, state=DGG.DISABLED, text=TTLocalizer.CogPageQuota % (count, quota), text_scale=0.045, text_fg=(0, 0, 0, 1), text_font=ToontownGlobals.getCogFont())
    ...
    def resetPanel(self, dept, type):
        panel = self.panels[dept * CogDNA.cogsPerDept + type]
    ...
    def setPanelStatus(self, panel, status):
        ...
        if status == COG_UNSEEN:
            ...
            if base.localAvatar.cogs[index] < COG_COMPLETE1:
                ...
                quota = str(COG_QUOTAS[1][index % CogDNA.cogsPerDept])
            panel.quotaLabel['text'] = TTLocalizer.CogPageQuota % (count, quota)
        elif status == COG_COMPLETE1:
            panel['image_color'] = PANEL_COLORS_COMPLETE1[index // CogDNA.cogsPerDept]
        elif status == COG_COMPLETE2:
            panel['image_color'] = PANEL_COLORS_COMPLETE2[index // CogDNA.cogsPerDept]
    ...
    def updateCogRadar(self, deptNum, panels, timeout = 0):
        ...
        for panel in panels:
            panel.cogRadarLabel['text'] = TTLocalizer.CogPageCogRadar % panel.count
            if self.radarOn[deptNum]:
                panel.quotaLabel.hide()
    ...
    def updateBuildingRadar(self, deptNum, timeout = 0):
        ...
        if button.building:
            ...
            if self.radarOn[deptNum]:
                ...
                if num == 1:
                    button.buildingRadarLabel['text'] = TTLocalizer.CogPageBuildingRadarS % num
                else:
                    button.buildingRadarLabel['text'] = TTLocalizer.CogPageBuildingRadarP % num

                def showLabel(button):
                    button.buildingRadarLabel.show()
                    button['state'] = DGG.NORMAL

                taskMgr.doMethodLater(RADAR_DELAY * CogDNA.cogsPerDept, showLabel, 'showBuildingRadarLater', extraArgs=(button,))
            else:
                button.buildingRadarLabel.hide()
        return
