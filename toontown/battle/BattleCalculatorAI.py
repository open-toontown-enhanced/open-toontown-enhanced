...
class BattleCalculatorAI:
    ...
    def __calcToonAtkHit(self, attackIndex, atkTargets):
        ...
        if atkTrack == TRAP:
            ...
            for tgt in atkTargets:
                if not self.__cogIsLured(tgt.getDoId()):
                    unluredCogs = 1
            ...
            for i in range(len(atkTargets)):
                if self.__cogIsLured(atkTargets[i].getDoId()):
                    pass
                else:
                    allLured = False
        ...
        if debug:
            if atkTrack == HEAL:
                ...
                self.notify.debug('Cog defense used for toon attack: ' + str(tgtDef))
    ...
    def __targetDefense(self, cog, atkTrack):
        if atkTrack == HEAL:
            return 0
        cogDef = CogBattleGlobals.CogAttributes[cog.dna.name]['def'][cog.getLevel()]
        return -cogDef
    ...
    def __createToonTargetList(self, attackIndex):
        ...
        if not attackAffectsGroup(atkTrack, atkLevel, attack[TOON_TRACK_COL]):
            if atkTrack == HEAL:
                ...
                target = self.battle.findCog(attack[TOON_TGT_COL])
            if target != None:
    ...
    def __cogTrapDamage(self, cogId):
        if cogId in self.traps:
            return self.traps[cogId][2]
        else:
            return 0

    def addTrainTrapForJoiningCog(self, cogId):
        self.notify.debug('addTrainTrapForJoiningCog cog=%d self.traps=%s' % (cogId, self.traps))
        trapInfoToUse = None
    ...
    def __addCogGroupTrap(self, cogId, trapLvl, attackerId, allCogs, npcDamage=0):
        if npcDamage == 0:
            ...
            if cogId in self.traps:
                ...
                if not self.__cogIsLured(cogId):
                    self.traps[cogId] = [
                     trapLvl, 0, npcDamage]
    ...
    def __calcToonAtkHp(self, toonId):
        ...
        for currTarget in range(len(targetList)):
            ...
            if atkTrack == LURE:
                if self.getCogTrapType(targetId) == NO_TRAP:
                    if self.notify.getDebug():
                        self.notify.debug('Cog lured, but no trap exists')
                    if self.COGS_UNLURED_IMMEDIATELY:
                        if not self.__cogIsLured(targetId, prevRound=1):
                            ...
                            currLureId = self.__addLuredCogInfo(targetId, -1, rounds, wakeupChance, toonId, atkLevel, lureId=currLureId, npc=npcLurer)
                            if self.notify.getDebug():
                                self.notify.debug('Cog lured for ' + str(rounds) + ' rounds max with ' + str(wakeupChance) + '% chance to wake up each round')
                            targetLured = 1
                else:
                    attackTrack = TRAP
                    if targetId in self.traps:
                        ...
                        attackLevel = NO_TRAP
                    attackDamage = self.__cogTrapDamage(targetId)
                    ...
                    lureDidDamage = 1
                    if self.notify.getDebug():
                        self.notify.debug('Cog lured right onto a trap! (' + str(AvProps[attackTrack][attackLevel]) + ',' + str(attackLevel) + ')')
                    if not self.__combatantDead(targetId, toon=toonTarget):
                        validTargetAvail = 1
                    targetLured = 1
                ...
                if atkTrack == TRAP:
                    ...
                    if atkLevel == UBER_GAG_LEVEL_INDEX:
                        ...
                        if self.__cogIsLured(targetId):
                            self.notify.debug('Train Trap on lured cog %d, \n indicating with KBBONUS_COL flag' % targetId)
                ...
                elif self.__cogIsLured(targetId) and atkTrack == SOUND:
                    self.notify.debug('Sound on lured cog, ' + 'indicating with KBBONUS_COL flag')
                    tgtPos = self.battle.activeCogs.index(targetList[currTarget])
                    attack[TOON_KBBONUS_COL][tgtPos] = self.KBBONUS_LURED_FLAG
                ...
                if attack[TOON_TRACK_COL] == NPCSOS and lureDidDamage != 1 or attack[TOON_TRACK_COL] == PETSOS:
                    ...
                    cog = self.battle.findCog(targetId)
                    if cog:
                        costToFire = 1
                ...
                if not self.__combatantDead(targetId, toon=toonTarget):
                    if self.__cogIsLured(targetId) and atkTrack == DROP:
                        self.notify.debug('not setting validTargetAvail, since drop on a lured cog')
                    else:
                        validTargetAvail = 1
            if attackLevel == -1 and not atkTrack == FIRE:
                ...
                result = attackDamage
                if atkTrack == HEAL:
                    if not self.__attackHasHit(attack, cog=0):
                        result = result * 0.2
                    if self.notify.getDebug():
                    ...
                    if self.__cogIsLured(targetId) and atkTrack == DROP:
                        result = 0
                        self.notify.debug('setting damage to 0, since drop on a lured cog')
                    if self.notify.getDebug():
                        self.notify.debug('toon does ' + str(result) + ' damage to cog')
            else:
                result = 0
    ...
    def __attackHasHit(self, attack, cog=0):
        if cog == 1:
    ...
    def __attackDamage(self, attack, cog=0):
        if cog:
            for dmg in attack[COG_HP_COL]:
                if dmg > 0:
                    return dmg
    ...
    def __attackDamageForTgt(self, attack, tgtPos, cog=0):
        if cog:
            ...
            for position in range(len(targets)):
                ...
                if currTarget.getHP() <= 0:
                    if currTarget.getSkeleRevives() >= 1:
                        ...
                        if self.notify.getDebug():
                            self.notify.debug('Cog' + str(targetId) + 'bravely expired in combat')
    ...
    def __clearTgtDied(self, tgt, lastAtk, currAtk):
        ...
        if currAtkTrack == lastAtkTrack and lastAtk[COG_DIED_COL] & 1 << position and self.__attackHasHit(currAtk, cog=0):
            if self.notify.getDebug():
                self.notify.debug('Clearing cog died for ' + str(tgt.getDoId()) + ' at position ' + str(position) + ' from toon attack ' + str(lastAtk[TOON_ID_COL]) + ' and setting it for ' + str(currAtk[TOON_ID_COL]))
            lastAtk[COG_DIED_COL] = lastAtk[COG_DIED_COL] ^ 1 << position
            self.cogLeftBattle(tgt.getDoId())
            ...
            if hp:
                if track in self.hpBonuses[tgtPos]:
                    ...
                    self.hpBonuses[tgtPos][track] = [
                     [
                      attackIndex, dmg]]
            elif self.__cogIsLured(currTgt.getDoId()):
    ...
    def __bonusExists(self, tgtCog, hp=1):
        tgtPos = self.activeCogs.index(tgtCog)
        if hp:
    ...
    def __handleBonus(self, attackIdx, hp=1):
        attackerId = self.toonAtkOrder[attackIdx]
        attack = self.battle.toonAttacks[attackerId]
        atkDmg = self.__attackDamage(attack, cog=0)
        atkTrack = self.__getActualTrack(attack)
        if atkDmg > 0:
            if hp:
                if atkTrack != LURE:
            ...
            if currToonAttack != -1:
                ...
                if atkTrack != HEAL and atkTrack != SOS and atkTrack != NO_ATTACK and atkTrack != NPCSOS and atkTrack != PETSOS:
                    ...
                    for currTgt in targets:
                        damageDone = self.__attackDamage(attack, cog=0)
                        if damageDone > 0:
                        ...
                        if tgtId in self.successfulLures and atkTrack == LURE:
                            ...
                            self.notify.debug('Drop on lured cog, ' + 'indicating with KBBONUS_COL ' + 'flag')
                            tgtPos = self.battle.activeCogs.index(currTgt)
                            attack[TOON_KBBONUS_COL][tgtPos] = self.KBBONUS_LURED_FLAG
    ...
    def __allTargetsDead(self, attackIdx, toon=1):
        ...
        if toon:
            ...
            self.notify.warning('__allTargetsDead: cog ver. not implemented!')
    ...
    def __clearLuredCogsByAttack(self, toonId, kbBonusReq=0, targetId=-1):
        ...
        if targetId != -1 and self.__cogIsLured(t.getDoId()):
            self.__removeLured(t.getDoId())
        else:
            tgtList = self.__createToonTargetList(toonId)
            for t in tgtList:
                if self.__cogIsLured(t.getDoId()) and (not kbBonusReq or self.__bonusExists(t, hp=0)):
                    self.__removeLured(t.getDoId())
                    if self.notify.getDebug():
                        self.notify.debug('Cog %d stepping from lured spot' % t.getDoId())
                else:
                    self.notify.debug('Cog ' + str(t.getDoId()) + ' not found in currently lured cogs')

    def __clearLuredCogsDelayed(self):
        if self.notify.getDebug():
            self.notify.debug('__clearLuredCogsDelayed')
        for t in self.delayedUnlures:
            if self.__cogIsLured(t):
                self.__removeLured(t)
                if self.notify.getDebug():
                    self.notify.debug('Cog %d stepping back from lured spot' % t)
            else:
                self.notify.debug('Cog ' + str(t) + ' not found in currently lured cogs')
    ...
    def __addLuredCogsDelayed(self, toonId, targetId=-1, ignoreDamageCheck=False):
        ...
        if targetId != -1:
            ...
            tgtList = self.__createToonTargetList(toonId)
            for t in tgtList:
                if self.__cogIsLured(t.getDoId()) and t.getDoId() not in self.delayedUnlures and (self.__attackDamageForTgt(self.battle.toonAttacks[toonId], self.battle.activeCogs.index(t), cog=0) > 0 or ignoreDamageCheck):
    ...
    def __calculateToonAttacks(self):
        ...
        for toonId in self.toonAtkOrder:
            ...
            if atkTrack != NO_ATTACK and atkTrack != SOS and atkTrack != NPCSOS:
                ...
                unlureAttack = self.__attackHasHit(attack, cog=0) and self.__unlureAtk(toonId, toon=1)
                if unlureAttack:
                    if lastAttack:
    ...
    def __calcCogAtkType(self, attackIndex):
        theCog = self.battle.activeCogs[attackIndex]
        attacks = CogBattleGlobals.CogAttributes[theCog.dna.name]['attacks']
        atk = CogBattleGlobals.pickCogAttack(attacks, theCog.getLevel())
        return atk

    def __calcCogTarget(self, attackIndex):
        ...
        if cogId in self.CogAttackers and random.randint(0, 99) < 75:
            ...
            if toonId == -1 or toonId not in self.battle.activeToons:
                return -1
            self.notify.debug('Cog attacking back at toon ' + str(toonId))
    ...
    def __pickRandomToon(self, cogId):
        ...
        if len(liveToons) == 0:
            self.notify.debug('No tgts avail. for cog ' + str(cogId))
            return -1
        chosen = random.choice(liveToons)
        self.notify.debug('Cog randomly attacking toon ' + str(self.battle.activeToons[chosen]))
        return chosen

    def __cogAtkHit(self, attackIndex):
        if self.cogsAlwaysHit:
            ...
            if self.cogsAlwaysMiss:
                return 0
        theCog = self.battle.activeCogs[attackIndex]
        atkType = self.battle.cogAttacks[attackIndex][COG_ATK_COL]
        atkInfo = CogBattleGlobals.getCogAttack(theCog.dna.name, theCog.getLevel(), atkType)
        atkAcc = atkInfo['acc']
        cogAcc = CogBattleGlobals.CogAttributes[theCog.dna.name]['acc'][theCog.getLevel()]
        acc = atkAcc
        randChoice = random.randint(0, 99)
        if self.notify.getDebug():
            self.notify.debug('Cog attack rolled ' + str(randChoice) + ' to hit with an accuracy of ' + str(acc) + ' (attackAcc: ' + str(atkAcc) + ' cogAcc: ' + str(cogAcc) + ')')
        if randChoice < acc:
            return 1
        return 0

    def __cogAtkAffectsGroup(self, attack):
    ...
    def __createCogTargetList(self, attackIndex):
        attack = self.battle.cogAttacks[attackIndex]
        targetList = []
        if attack[COG_ATK_COL] == NO_ATTACK:
            ...
            return targetList
        debug = self.notify.getDebug()
        if not self.__cogAtkAffectsGroup(attack):
            targetList.append(self.battle.activeToons[attack[COG_TGT_COL]])
            if debug:
                self.notify.debug('Cog attack is single target')
        else:
            if debug:
                self.notify.debug('Cog attack is group target')
            for currToon in self.battle.activeToons:
                if debug:
                    self.notify.debug('Cog attack will target toon' + str(currToon))
                targetList.append(currToon)
        ...
        for currTarget in range(len(targetList)):
            ...
            if toon and toon.immortalMode:
                ...
                if self.TOONS_TAKE_NO_DAMAGE:
                    ...
                    if self.__cogAtkHit(attackIndex):
                        atkType = attack[COG_ATK_COL]
                        theCog = self.battle.findCog(attack[COG_ID_COL])
                        atkInfo = CogBattleGlobals.getCogAttack(theCog.dna.name, theCog.getLevel(), atkType)
                        result = atkInfo['hp']
    ...
    def __cogCanAttack(self, cogId):
        if self.__combatantDead(cogId, toon=0) or self.__cogIsLured(cogId) or self.__combatantJustRevived(cogId):
            return 0
        return 1

    def __updateCogAtkStat(self, toonId):
        if toonId in self.cogAtkStats:
            self.cogAtkStats[toonId] += 1
        else:
            self.cogAtkStats[toonId] = 1

    def __printCogAtkStats(self):
        self.notify.debug('Cog Atk Stats:')
        for currTgt in list(self.cogAtkStats.keys()):
            if currTgt not in self.battle.activeToons:
                continue
            tgtPos = self.battle.activeToons.index(currTgt)
            self.notify.debug(' toon ' + str(currTgt) + ' at position ' + str(tgtPos) + ' was attacked ' + str(self.cogAtkStats[currTgt]) + ' times')
    ...
    def __calculateCogAttacks(self):
        for i in range(len(self.battle.cogAttacks)):
            if i < len(self.battle.activeCogs):
                ...
                self.battle.cogAttacks[i][COG_ID_COL] = cogId
                if not self.__cogCanAttack(cogId):
                    if self.notify.getDebug():
                        self.notify.debug("Cog %d can't attack" % cogId)
                    continue
                ...
                attack[COG_ATK_COL] = self.__calcCogAtkType(i)
                attack[COG_TGT_COL] = self.__calcCogTarget(i)
                ...
                self.__calcCogAtkHp(i)
                if attack[COG_ATK_COL] != NO_ATTACK:
                    if self.__cogAtkAffectsGroup(attack):
                        for currTgt in self.battle.activeToons:
                            self.__updateCogAtkStat(currTgt)

                    else:
                        tgtId = self.battle.activeToons[attack[COG_TGT_COL]]
                        self.__updateCogAtkStat(tgtId)
                targets = self.__createCogTargetList(i)
                allTargetsDead = 1
                for currTgt in targets:
                ...
                if self.notify.getDebug():
                    self.notify.debug('Cog attack: ' + str(self.battle.cogAttacks[i]))
    ...
    def __updateLureTimeouts(self):
        ...
        noLongerLured = []
        for currLuredCog in list(self.currentlyLuredCogs.keys()):
            self.__incLuredCurrRound(currLuredCog)
            if self.__luredMaxRoundsReached(currLuredCog) or self.__luredWakeupTime(currLuredCog):
                noLongerLured.append(currLuredCog)

        for currLuredCog in noLongerLured:
            self.__removeLured(currLuredCog)
    ...
    def __initRound(self):
        ...
        if self.notify.getDebug():
            ...
            self.notify.debug('Cog attacks: ' + str(self.battle.cogAttacks))
        self.toonHPAdjusts = {}
        for t in self.battle.activeToons:
    ...
    def calculateRound(self):
        ...
        for i in range(4):
            for j in range(len(self.battle.activeToons)):
                self.battle.cogAttacks[i][COG_HP_COL].append(-1)

        toonsHit, cogsMiss = self.__initRound()
        for cog in self.battle.activeCogs:
            if cog.isGenerated():
                cog.b_setHP(cog.getHP())

        for cog in self.battle.activeCogs:
            if not hasattr(cog, 'dna'):
                self.notify.warning('a removed cog is in this battle!')
                return None
    ...
    def __cogIsLured(self, cogId, prevRound=0):
    ...
    def __addLuredCogInfo(self, cogId, currRounds, maxRounds, wakeChance, lurer, lureLvl, lureId=-1, npc=0):
        if lureId == -1:
        ...
        self.notify.debug('__addLuredCogInfo: currLuredCogs -> %s' % repr(self.currentlyLuredCogs))
        return availLureId

    def __getLurers(self, cogId):
        if self.__cogIsLured(cogId):
    ...
    def __luredMaxRoundsReached(self, cogId):
        return self.__cogIsLured(cogId) and self.currentlyLuredCogs[cogId][0] >= self.currentlyLuredCogs[cogId][1]

    def __luredWakeupTime(self, cogId):
        return self.__cogIsLured(cogId) and self.currentlyLuredCogs[cogId][0] > 0 and random.randint(0, 99) < self.currentlyLuredCogs[cogId][2]
